...
# -*- coding: utf-8 -*-
"""
Apex GIF Maker (MP4 → GIF), PySide6 + ffmpeg

v4.3 요약
- 구간 트림: 듀얼 핸들(2~10초 강제) + 시작/끝 시간 직접 입력(mm:ss.mmm / hh:mm:ss.mmm)
- 스케일 모드: 레터박스(비율 유지), 꽉 채우기(크롭, 기본), 스트레치(왜곡)
- 프리뷰: 좌/우 2분할(시작/끝), 각 1280x720로 추출. 슬라이더/시간 입력 시 자동 갱신(디바운스 200ms)
- 타임라인 썸네일: 2초 간격 스트립(스크롤)
- 캐시: ./cache, ffmpeg-bin: ./ffmpeg-bin (영구)
- Windows에서 ffmpeg/ffprobe 실행 시 콘솔창 비표시
"""

import os, sys, shutil, subprocess, platform, urllib.request, zipfile, tarfile
...
APP_DIR   = app_root()
CACHE_DIR = APP_DIR / "cache";        CACHE_DIR.mkdir(parents=True, exist_ok=True)
FFMPEG_DIR= APP_DIR / "ffmpeg-bin";   FFMPEG_DIR.mkdir(parents=True, exist_ok=True)
...
def build_gif_commands_auto(ffmpeg_path: str, video_path: str, start: float, end: float,
    ...
    if duration <= 0: raise ValueError("시작/끝 시간이 올바르지 않습니다.")
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.lbl_duration = QLabel("길이: -"); self.lbl_tools = QLabel(self._tools_label_text())
        ...
        self.le_start = QLineEdit("00:00.000"); self.le_end = QLineEdit("00:02.000")
        ...
        self.btn_prev_start = QPushButton("시작 프레임"); self.btn_prev_end = QPushButton("끝 프레임")
        ...
        tg.addWidget(self.btn_prev_start, 2, 4); tg.addWidget(self.btn_prev_end, 2, 5)

        split = QHBoxLayout(); split.addWidget(self.lbl_prev_start); split.addWidget(self.lbl_prev_end)
        tg.addLayout(split, 3, 0, 1, 6)

        # Options
        opt_group = QGroupBox("옵션"); og = QGridLayout(opt_group)
        ...
        self.combo_scale = QComboBox(); self.combo_scale.addItems(["레터박스(비율 유지)", "꽉 채우기(크롭)", "스트레치(왜곡)"]); self.combo_scale.setCurrentIndex(1)
        ...
        # Manual picker
        manual_group = QGroupBox("프레임 선택(수동 모드)"); mg = QVBoxLayout(manual_group)
        top_bar = QHBoxLayout()
        ...
        top_bar.addWidget(QLabel("스캔 FPS:")); top_bar.addWidget(self.spin_scan_fps); top_bar.addStretch(1); top_bar.addWidget(self.btn_scan); top_bar.addWidget(btn_all)
        self.list_frames = QListWidget(); self.list_frames.setViewMode(QListView.IconMode)
        self.list_frames.setIconSize(QSize(192,108)); self.list_frames.setResizeMode(QListView.Adjust); self.list_frames.setSpacing(6); self.list_frames.setMinimumHeight(220)
        ...
        # Run
        run_box = QHBoxLayout(); self.btn_generate = QPushButton("GIF 생성"); self.btn_generate.clicked.connect(self._generate_gif)
        ...
        # assemble
        root.addLayout(file_box); root.addLayout(meta_box); root.addWidget(trim_group); root.addWidget(opt_group); root.addWidget(manual_group)
        root.addLayout(out_box); root.addLayout(run_box); root.addWidget(self.log, 1)
    ...
    def _auto_setup_tools(self):
        self.ffmpeg_path = find_executable("ffmpeg"); self.ffprobe_path = find_executable("ffprobe")
        if self.ffmpeg_path and self.ffprobe_path:
            self._append_log("[INFO] ffmpeg/ffprobe 준비됨"); self.lbl_tools.setText(self._tools_label_text()); return
    ...
    def _load_video(self, path: str):
        ...
        try:
            ...
            self.range.setRange(0.0, hi); self._apply_trim_constraints(adjust='end')
    ...
    def _on_range_changed(self, lo: float, hi: float):
        self._apply_trim_constraints(); self._update_time_edits(); self.preview_timer.start()
    ...
    def _apply_edits_to_range(self):
        ...
        lo = max(0.0, lo); hi = min(self.duration_sec, hi)
    ...
    def _build_timeline_thumbs(self):
        ...
        for fp in sorted(thumbs_dir.glob("thumb_*.png")):
            lb = QLabel(); lb.setPixmap(QPixmap(str(fp)).scaled(96,54, Qt.KeepAspectRatio, Qt.SmoothTransformation))
    ...
    def _scan_frames(self):
        ...
        cmd = [self.ffmpeg_path, "-hide_banner", "-loglevel", "error", "-ss", f"{lo:.3f}", "-t", f"{duration:.3f}",
               "-i", self.video_path, "-vf", f"fps={scan_fps},scale=384:216:flags=lanczos", str(frames_dir / "frame_%04d.png")]
    ...
    def _generate_gif(self):
        ...
        if mode_idx in (0,1):
            ...
            frames_dir = CACHE_DIR / "frames_selected"; self._clear_frames_dir(frames_dir)
        ...
        self.btn_generate.setEnabled(False); self._append_log("[RUN] GIF 생성 시작")
