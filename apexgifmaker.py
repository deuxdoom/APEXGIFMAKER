...
(배포 시 README/ATTRIBUTION에 저작권 고지 필요)
"""
...
GITHUB_HTML_LATEST = f"https://github.com/deuxdoom/APEXGIFMAKER/releases/latest"
...
# =========================================================
# 내장 아이콘(Base64 PNG)
# - 임의의 파란색 아이콘 PNG(Base64). 필요 시 Flaticon 아이콘의 PNG를 Base64로 교체 가능.
# - 교체 방법: 원하는 png → base64 인코딩 → 아래 문자열 대치
# =========================================================
EMBED_ICON_PNG_B64 = (    
...
class MainWindow(QMainWindow):
    ...
    def _normalize_version(self, v: str) -> tuple:
        """
        'v1.2.3' / '1.2.3' → (1,2,3). 예외 시 ().
        접두 v/V 제거, 숫자 외 접미(예: rc) 무시.
        """
        try:
            v = v.strip()
            if v.startswith(("v","V")):
    ...
    def _check_for_updates(self, manual: bool = False):
        ...
        manual=True: 결과(최신/에러) 알림, False: 신버전만 알림.
        """
        try:
            req = urllib.request.Request(
                GITHUB_API_LATEST,
                headers={
                    "User-Agent": f"ApexGIFMaker/{APP_VERSION}",
                    "Accept": "application/vnd.github+json",
                },
            )
            with urllib.request.urlopen(req, timeout=5) as resp:
                data = json.loads(resp.read().decode("utf-8", "ignore"))
            tag = str(data.get("tag_name", "")).strip()  # e.g., "v1.2.0"
            if not tag:
                if manual:
                    QMessageBox.information(self, "업데이트", "업데이트 정보를 가져오지 못했습니다.")
                return

            cur = self._normalize_version(APP_VERSION)
            new = self._normalize_version(tag)
            if not cur or not new:
                if manual:
                    QMessageBox.information(self, "업데이트", "버전 정보 형식이 올바르지 않습니다.")
                return

            if new > cur:
                m = QMessageBox(self)
                m.setIcon(QMessageBox.Information)
                m.setWindowTitle("업데이트")
                m.setText("새로운 버전이 나왔습니다.\n다운하러 이동하시겠습니까?")
                m.setInformativeText(f"(현재: v{APP_VERSION} / 최신: {tag})")
            ...
            if manual:
                QMessageBox.warning(self, "업데이트", f"업데이트 확인 실패:\n{e}")
